...
twitchBot.then(({ twitchListener, twitchChat, twitchAPI }) => { 
  ...
  app.get("/api/rewardsList", (req, res) => {
    twitchAPI.channelPoints.getCustomRewards(twitchAuth.channelID, false).then(response => {
  ...
  app.get("/api/obsItemsList", (req, res) => {
    ...
    obs.client.call("GetSceneItemList", {"sceneName" : sceneName}).then(list => {
  ...
  app.get("/api/obsFiltersItemList", (req, res) => {
    ...
    obs.client.call("GetSourceFilterList", {"sourceName" : sourceName}).then(list => {
